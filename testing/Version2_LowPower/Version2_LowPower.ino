...
#define WAKE_BUTTON GPIO_NUM_3 // GPIO3 as wake-up source
#define LIGHT_SLEEP_TIMEOUT 60000 // 60 seconds
...
// DONT FORGET TO CHANGE THIS LINK
const char* apiEndpoint = "https://app.beeceptor.com/console/firefly-launch"; // DONT FORGET TO CHANGE THIS LINK
// DONT FORGET TO CHANGE THIS LINK
// DONT FORGET TO CHANGE THIS LINK
// DONT FORGET TO CHANGE THIS LINK
// DONT FORGET TO CHANGE THIS LINK
...
volatile int animationStartTime = 0;
#define ANIMATION_UPTIME 60000
#define ANIMATION_DOWNTIME 20000
static bool launch_timer_initiated = true;
...
void setup() {
    ...
    // Connect to Wifi
    connectToWifi();
    // Get Launch Parameters from API Call
    if(GetLaunchParameters()){
      Serial.print("Got Latest Launch Parameters");
    }
    // Setup Internal Time by syncing with NTP Server
    setupTime();
    // Turn OFF WiFi after syncing time
...
void loop() {
  ...
  case LAUNCH_TIMER:
    /* code */
    if(launch_timer_initiated){
      animationStartTime = millis();
      launch_timer_initiated = false;
    }
    if(millis() - animationStartTime >= ANIMATION_UPTIME) {
      menuState = SLEEP;
      launch_timer_initiated = true;
    }
    LaunchTimer();
    break;
  ...
  if(launch_timer_initiated){
    ...
    if(millis() - animationStartTime >= ANIMATION_DOWNTIME) {
